...
const defaultKoboldApiBase = 'http://localhost:5001';

async function getKoboldApiEndpoint(): Promise<string> {
  const savedBase = await getSetting(API_BASE_URL_KEY);
  const base = savedBase?.trim() || defaultKoboldApiBase;
  // If the base already ends with /api keep it as-is, otherwise append /api.
...
export async function callKoboldApiWithHistory(prompt: string, history: string) {
  ...
  const endpoint = await getKoboldApiEndpoint();
