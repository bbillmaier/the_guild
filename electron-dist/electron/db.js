...
const mundane_items_seed_1 = require("../lib/mundane-items-seed");
const phys_desc_seed_1 = require("../lib/phys-desc-seed");
const roleplay_seeds_1 = require("../lib/roleplay-seeds");
const group_greeting_seeds_1 = require("../lib/group-greeting-seeds");
const guild_event_seeds_1 = require("../lib/guild-event-seeds");
// ─── State ────────────────────────────────────────────────────────────────────
let _db = null;
let _dbPath = '';
function getDb() {
...
function persist() {
    ...
    fs_1.default.writeFileSync(_dbPath, Buffer.from(data));
}
...
async function initDatabase() {
    ...
    _dbPath = path_1.default.join(userDataPath, 'guild.db');
    ...
    const sqlJsDistDir = path_1.default.dirname(require.resolve('sql.js'));
    const SQL = await initSqlJs({
        locateFile: (file) => {
            // In a packaged ASAR, WASM files are unpacked to app.asar.unpacked/
            const packed = path_1.default.join(sqlJsDistDir, file);
            const unpacked = packed.replace('app.asar' + path_1.default.sep, 'app.asar.unpacked' + path_1.default.sep);
            return fs_1.default.existsSync(unpacked) ? unpacked : packed;
        },
    });
    let dbData;
    try {
        dbData = fs_1.default.readFileSync(_dbPath);
    }
    catch { /* first run */ }
...
function runMigrations() {
  ...
  `);
    let version = (_get('SELECT version FROM schema_migrations WHERE id = 1;'))?.version ?? 0;
    function bump(n, fn) {
        if (version < n) {
            fn();
            version = n;
...
function migrateV2() {
    const cols = _all(`PRAGMA table_info(characters)`);
...
function migrateV5() {
    ...
    const rows = _all('SELECT uid, constitution FROM characters;');
...
function migrateV7() {
    const cc = _all(`PRAGMA table_info(characters)`);
    if (!cc.some((c) => c.name === 'level'))
        _exec(`ALTER TABLE characters ADD COLUMN level INTEGER NOT NULL DEFAULT 1;`);
    if (!cc.some((c) => c.name === 'experience'))
        _exec(`ALTER TABLE characters ADD COLUMN experience INTEGER NOT NULL DEFAULT 0;`);
    const ec = _all(`PRAGMA table_info(enemies)`);
    if (!ec.some((c) => c.name === 'level'))
        _exec(`ALTER TABLE enemies ADD COLUMN level INTEGER NOT NULL DEFAULT 1;`);
}
function migrateV8() {
    _exec(`
...
function migrateV9() {
    const cols = _all(`PRAGMA table_info(items)`);
...
function migrateV11() {
    const cols = _all(`PRAGMA table_info(quests)`);
...
function migrateV12() {
  ...
  `);
    ...
    for (const s of seeds)
        _run(`INSERT OR IGNORE INTO roleplay_prompts (uid, text, category, active) VALUES (?, ?, ?, 1);`, [s.uid, s.text, s.cat]);
}
function migrateV13() {
    const cols = _all(`PRAGMA table_info(quests)`);
...
function migrateV16() {
    const cols = _all(`PRAGMA table_info(chat_history)`);
...
function migrateV17() {
  ...
  `);
    ...
    for (const s of seeds)
        _run(`INSERT OR IGNORE INTO character_greetings (uid, room, message) VALUES (?, ?, ?);`, [s.uid, s.room, s.msg]);
}
function migrateV18() {
    const cols = _all(`PRAGMA table_info(characters)`);
...
function migrateV23() {
    const cols = _all(`PRAGMA table_info(outfit_sets)`);
...
function migrateV28() {
    ...
    for (const s of roleplay_seeds_1.ROLEPLAY_SEEDS) {
        _run(`UPDATE roleplay_prompts SET text = ?, relationship_delta = ? WHERE uid = ?;`, [s.text, s.relationshipDelta, s.uid]);
    }
}
function migrateV29() {
...
function migrateV30() {
    ...
    for (const s of group_greeting_seeds_1.GROUP_GREETING_SEEDS) {
...
function migrateV31() {
    ...
    for (const s of guild_event_seeds_1.GUILD_EVENT_SEEDS) {
        _run(`INSERT OR IGNORE INTO guild_event_seeds (uid, text, active, relationship_delta, use_common_quest) VALUES (?, ?, 1, ?, ?);`, [s.uid, s.text, s.relationshipDelta, s.useCommonQuest ? 1 : 0]);
    }
}
function migrateV32() {
...
function migrateV33() {
    const cols = _all(`PRAGMA table_info(quest_history)`);
    if (!cols.some((c) => c.name === 'transcript'))
...
function ensurePhysDescSeeds() {
    let inserted = 0;
    for (const [category, value] of phys_desc_seed_1.PHYS_DESC_SEED) {
        try {
            const r = _run(`INSERT OR IGNORE INTO phys_desc_pool (category, value) VALUES (?, ?);`, [category, value]);
            inserted += r.changes;
        }
        catch (e) {
            console.error('[seed] phys_desc_pool:', e);
        }
    }
    if (inserted > 0)
        console.log(`[seed] phys_desc_pool: inserted ${inserted} new entries`);
}
function seedMundaneItems() {
    let inserted = 0;
    for (const item of mundane_items_seed_1.STARTER_MUNDANE_ITEMS) {
        try {
            ...
            console.error('[seed] Failed to insert:', item.name, e);
        }
    }
    console.log(`[seed] Inserted ${inserted} / ${mundane_items_seed_1.STARTER_MUNDANE_ITEMS.length} mundane items`);
}
function ensureMetaDescSeeds() {
    ...
    const sc = ['intelligence', 'wisdom', 'charisma', null];
    const mc = [1, -1, null, null];
    for (let i = 0; i < 100; i++) {
...
function registerDbHandlers() {
    electron_1.ipcMain.handle('db:all', (_event, sql, params) => {
        try {
            return _all(sql, params);
        }
        catch (err) {
            console.error('[db:all]', sql, err);
            throw err;
        }
    });
    electron_1.ipcMain.handle('db:get', (_event, sql, params) => {
        try {
            return _get(sql, params) ?? null;
        }
        catch (err) {
    ...
    electron_1.ipcMain.handle('db:run', (_event, sql, params) => {
        try {
            ...
            console.error('[db:run]', sql, err);
            throw err;
        }
    });
    electron_1.ipcMain.handle('db:exec', (_event, sql) => {
        try {
            _exec(sql);
            persist();
            return null;
...
function registerFsHandlers() {
    electron_1.ipcMain.handle('fs:getUserDataPath', () => electron_1.app.getPath('userData'));
    electron_1.ipcMain.handle('fs:saveImage', async (_event, imageUrl, subfolder, filename) => {
        const https = await Promise.resolve().then(() => __importStar(require('https')));
        const http = await Promise.resolve().then(() => __importStar(require('http')));
        const dir = path_1.default.join(electron_1.app.getPath('userData'), subfolder);
        ...
        const relPath = path_1.default.relative(electron_1.app.getPath('userData'), localPath).replace(/\\/g, '/');
